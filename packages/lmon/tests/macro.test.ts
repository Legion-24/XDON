import { expand, MacroContext, LMONMacroError } from '../src';
...
describe('macro: simple variables', () => {
  ...
  test('A4: forward ref error', () => {
    const input = '%x\n%x = "a"';
    expect(() => expand(input)).toThrow(LMONMacroError);
  });

  test('A5: embedded in LMON', () => {
...
describe('macro: nesting', () => {
  ...
  test('C4: max depth limit', () => {
    ...
    input += '%a19';
    expect(() => expand(input, { maxDepth: 16 })).toThrow(LMONMacroError);
  });
});
...
describe('macro: expressions', () => {
  ...
  test('D12: division by zero', () => {
    expect(() => expand('%{5/0}')).toThrow(LMONMacroError);
  });

  test('D13: invalid expression', () => {
    expect(() => expand('%{abc}')).toThrow(LMONMacroError);
  });

  test('D14: unclosed brace', () => {
    expect(() => expand('%{5+3')).toThrow(LMONMacroError);
  });
...
describe('macro: errors with line tracking', () => {
  test('E1: undefined macro strict mode', () => {
    const input = '(name)\n%noexist';
    expect(() => expand(input, { strict: true })).toThrow(LMONMacroError);
  });
...
describe('macro: integration with LMON parsing', () => {
