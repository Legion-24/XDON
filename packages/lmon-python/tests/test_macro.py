import pytest
from lmon import expand, MacroContext, MacroDefinition, ExpandOptions, LMONMacroError, parse
...
class TestSimpleVariableMacros:
    ...
    def test_a4_forward_ref_error(self):
        input_text = '%x\n%x = "a"'
        with pytest.raises(LMONMacroError):
            expand(input_text)

    def test_a5_embedded_in_lmon(self):
...
class TestIntegrationWithLMONParsing:
