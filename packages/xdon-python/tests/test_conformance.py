...
These tests mirror packages/xdon/tests/conformance.test.ts and must pass
...
from xdon.evaluator import infer_type
...
class TestReservedLeading:
    def test_at_rejected(self):
        with pytest.raises(XDONParseError):
            parse("{@user}")
...
class TestVersionDirective:
    def test_v10_accepted(self):
        assert parse("!XDON 1.0\n{1}") == [[1]]

    def test_v15_accepted(self):
        assert parse("!XDON 1.5\n{1}") == [[1]]
...
class TestStringifier:
    ...
    def test_cyclic_input_raises(self):
        o: dict = {"a": 1}
        o["self"] = o
        with pytest.raises(XDONStringifyError):
            stringify({"row": o})
