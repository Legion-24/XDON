import { XDONMacroError } from './errors.js';
...
class ExprEvaluator {
  ...
  private expect(type: ExprTokenType, line: number, col: number): ExprToken {
    ...
    if (token.type !== type) {
      throw new XDONMacroError(`Expected ${type} but got ${token.type}`, line, col);
    }
    return this.advance();
  }
  ...
  private parseMulExpr(line: number, col: number): number {
    ...
    while (
      ...
      if (op.type === ExprTokenType.STAR) left *= right;
        ...
        if (right === 0) throw new XDONMacroError('Division by zero in expression', line, col);
        left /= right;
      } else {
        if (right === 0) throw new XDONMacroError('Modulo by zero in expression', line, col);
        left %= right;
      }
    }
    return left;
...
function expandLine(
  ...
  if (depth > state.maxDepth) {
    throw new XDONMacroError('Macro expansion depth exceeded', lineNum, 1);
  }
